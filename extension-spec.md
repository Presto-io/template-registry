  ...
  "templates": [
    {
      ...
      "keywords": ["公文", "国标"],
	      "license": "MIT",
	      "trust": "official",
	      "manifest_url": "https://presto.c-1o.top/templates/gongwen/manifest.json",
...
template-registry/
  templates/
    gongwen/
      manifest.json          ← 源码/发布产物中的声明文件，不要求安装端执行二进制提取
      README.md              ← 从模板仓库首页获取
...
后端：
  ...
  7. 下载/读取 registry 声明的 manifest.json，校验 manifest_sha256、name、version
  8. 写入 staging 目录：manifest.json、install-lock.json、当前平台 artifact
  9. 原子替换 ~/.presto/templates/{name}/，失败时保留旧版本
  10. legacy/discovery 安装才允许回退到 ./binary --manifest
```
...
~/.presto/
  templates/
    {name}/
      manifest.json                    ← registry/独立 manifest 声明文件
      install-lock.json                ← 本机安装事实：artifact URL/hash/platform/source
      presto-template-{name}[.exe]     ← 可执行文件（Windows 可保留平台后缀）
  plugins/                             ← 将来
    {name}/
      ...
```
